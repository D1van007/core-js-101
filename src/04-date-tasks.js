...
function timeSpanToString(startDate, endDate) {
  ...
  const hour = `0${Math.floor(diff / 3600000)}`;
  // eslint-disable-next-line no-mixed-operators
  const min = `0${(Math.floor(diff % 3600000 / 60000))}`;
  // eslint-disable-next-line no-mixed-operators
  const sec = `0${(Math.floor(diff % 60000 / 1000))}`;
  const mlsec = `00${diff % 3600000}`;
  return `${hour.slice(-2)}:${min.slice(-2)}:${sec.slice(-2)}.${mlsec.slice(-3)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  let minutesHand = (360*minutes)/60;
  let hoursHand = (360*(hours+minutes/60))/12;

  
  let angle= Math.abs(minutesHand-hoursHand);
  if(angle>180) {
